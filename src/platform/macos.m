...
// Plugin launcher source (issue #323) lives in macos_plugin.m and is
// included here — one TU, so the -Oz machine outliner sees the same code
// as before the split and ship __TEXT stays byte-identical (AGENTS.md §7).
// READ_PLUGIN_STUB=1 (twin builds, -Dplugin_stub) includes the empty stub
// instead; same TU, same flags. Nothing plugin-shaped remains inline here.
#if READ_PLUGIN_STUB
#include "macos_plugin_stub.m"
#else
