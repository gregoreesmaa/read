...
/// VoiceOver notification contract (mirrored by ax_notify_selection_changed
/// + ax_note_draw_locked in src/platform/macos.m). Posting is cheap but
/// must be exact: AT clients re-read on every event, so unchanged state
/// posts nothing. Pure model, zero heap.
pub const A11yLog = struct {
    value_changed: u32 = 0,
    selected_text_changed: u32 = 0,
    last_text_len: usize = 0,
    last_text_count: usize = 0,

    /// End-of-draw check: scalar compare only. Posts AXValueChanged iff
    /// the record count or total length moved (new document text).
    pub fn noteDrawnText(self: *A11yLog, record_count: usize, total_len: usize) void {
        if (record_count != self.last_text_count or total_len != self.last_text_len) {
            self.last_text_count = record_count;
            self.last_text_len = total_len;
            self.value_changed += 1;
        }
    }

    /// Selection commit: posts AXSelectedTextChanged iff the selection
    /// actually changed. The word/line mouseUp lock passes false.
    pub fn noteSelectionCommitted(self: *A11yLog, changed: bool) void {
        if (changed) self.selected_text_changed += 1;
    }
};

/// AX role description for a block (mirrors accessibilityChildren in
/// macos.m): headings announce as "heading", code blocks as "code block",
/// body text reads as continuous text with no per-run role.
pub fn axRoleDescriptionForBlock(is_heading: bool, is_code: bool) ?[]const u8 {
    if (is_heading) return "heading";
    if (is_code) return "code block";
    return null;
}

test "controls: voiceover posts only on real selection or text change" {
    var log = A11yLog{};

    // First draw with document text: one value event.
    log.noteDrawnText(10, 512);
    try std.testing.expectEqual(@as(u32, 1), log.value_changed);

    // Steady redraws (scroll, hover): nothing.
    log.noteDrawnText(10, 512);
    log.noteDrawnText(10, 512);
    try std.testing.expectEqual(@as(u32, 1), log.value_changed);

    // New document text: one more value event.
    log.noteDrawnText(12, 600);
    try std.testing.expectEqual(@as(u32, 2), log.value_changed);

    // Click/drag/select-all commits: one selection event each.
    log.noteSelectionCommitted(true);
    log.noteSelectionCommitted(true);
    try std.testing.expectEqual(@as(u32, 2), log.selected_text_changed);

    // Word/line mouseUp lock: no change, no event.
    log.noteSelectionCommitted(false);
    try std.testing.expectEqual(@as(u32, 2), log.selected_text_changed);

    // Role mapping for the rotor children.
    try std.testing.expectEqualStrings("heading", axRoleDescriptionForBlock(true, false).?);
    try std.testing.expectEqualStrings("code block", axRoleDescriptionForBlock(false, true).?);
    try std.testing.expect(axRoleDescriptionForBlock(false, false) == null);
    // A heading code span still announces as a heading.
