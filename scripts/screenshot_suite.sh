...
# Case 4l: Math rendered image (issue #326, KaTeX) — read-test never
...
math_hash=$(python3 - src/core/plugin_cache.zig test_cases/plugin_math.md math math <<'EOF'
import re, sys
...
[ -n "$math_hash" ] || { echo "FAIL: case 4l fence hash empty" >&2; exit 1; }
if [ -n "${HOME:-}" ]; then cache_root="$HOME/Library/Caches"; else cache_root="${TMPDIR:-/tmp}/read-plugin-cache"; fi
