#!/bin/sh
# Binary footprint gate (see AGENTS.md §1: executable strictly under 350 KiB).
#
# Fails when the ship binary reaches 350 KiB (358400 bytes). Also reports
...
fail=0
if [ "$bytes" -ge 358400 ]; then
    echo "FAIL: ship binary is $bytes bytes (>= 350 KiB budget)"
    fail=1
fi
...
if [ "$fail" = 0 ]; then
    echo "PASS: binary footprint under 350 KiB"
fi
exit $fail
