#!/bin/sh
# Binary footprint gate (see AGENTS.md §1: executable strictly under 200 KiB).
#
# Fails when the ship binary reaches 200 KiB (204800 bytes). Also reports
...
if [ "$bytes" -ge 204800 ]; then
    echo "FAIL: ship binary is $bytes bytes (>= 200 KiB budget)"
    fail=1
fi
...
if [ "$fail" = 0 ]; then
    echo "PASS: binary footprint under 200 KiB"
fi
exit $fail
